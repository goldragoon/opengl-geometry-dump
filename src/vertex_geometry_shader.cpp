...
bool vertex_geometry_shader::init(const char* vertex_shader_filename, const char* geometry_shader_filename, vector<const char*>& varying_names)
{
	...
	if (varying_names.size() > 0)
	{
		const GLchar** feedbackVaryings = &(varying_names[0]);
